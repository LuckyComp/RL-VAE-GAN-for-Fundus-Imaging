...
def generate_random_images(generator, device, output_dir, num_images=16, latent_dim=256):
    """Generates images from pure random noise."""
    ...
    with torch.no_grad():
        ...
        # Generate and denormalize [-1, 1] -> [0, 1]
...
def generate_from_encoder(generator, encoder, dataloader, device, output_dir):
    """Generates images by passing a real batch through the VAE encoder."""
    ...
    with torch.no_grad():
        ...
        # Save both grids
        save_image(real_images, os.path.join(output_dir, "inference_real_inputs.png"), nrow=4)
        save_image(fake_images, os.path.join(output_dir, "inference_encoded_fakes.png"), nrow=4)
...
if __name__ == "__main__":
    ...
    generator.load_state_dict(torch.load(GENERATOR_WEIGHTS, map_location=DEVICE, weights_only=True))
    generator.eval()
    print("Generator weights loaded successfully.")

    # 3. Generate completely random retinas
    generate_random_images(generator, DEVICE, OUTPUT_DIR)
