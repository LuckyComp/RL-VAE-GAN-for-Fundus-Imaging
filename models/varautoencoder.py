...
class ResidualBlock(
    nn.Module
):  # Learns the difference between the transformation and original image
    def __init__(self, channels):
        ...
        self.block = nn.Sequential(
            ...
            ),  # Transformations on the activated outputs
            nn.BatchNorm2d(channels),  # Normalize
        )
    ...
    def forward(self, x):
        return self.activation(
            x + self.block(x)
        )  # Activation with the skip where x is the input and self.block(x) is the residual
...
class Encoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=256, base_channels=64):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(
                base_channels * 4, base_channels * 8, 4, stride=2, padding=1
            ),  # Block 4
            nn.LeakyReLU(0.2, True),
        ...
        self.adaptive_pool = nn.AdaptiveAvgPool2d(
            (4, 4)
        )  # Ensures regardless of input dimensions the final output is 4 x 4 using averaging
        flat_dim = (
            base_channels * 8 * 4 * 4
        )  # What the size of the flattened dimensions should be after transformations
        self.fc_mu = nn.Linear(
            flat_dim, latent_dim
        )  # Calculates mean of guassian distribution
        self.fc_log_var = nn.Linear(
            flat_dim, latent_dim
        )  # Calculates +ve variance values using log_var

    def forward(self, x):
        x = self.encoder(x)
        x = self.adaptive_pool(x)
        x = x.flatten(start_dim=1)  # Resizes from 3D vector to 1D vector of size 8192
...
def reparameterize(mu, log_var):
    ...
    return (
        mu + std * eps
    )  # Introduces subtle shift from fixed points in the latent distribution
...
class Decoder(nn.Module):
    def __init__(self, out_channels=3, latent_dim=256, base_channels=64):
        ...
        self.decode = nn.Sequential(
            ...
            ResidualBlock(base_channels),
            nn.ConvTranspose2d(
                base_channels, out_channels, 4, stride=2, padding=1
            ),  # Upscale to out_channels
...
class VAE(nn.Module):
    ...
    def infer(self, x):  # Used at inference time
        ...
        return (
            ...
            mu,
            log_var,
        )  # Only returns Z value and mu, var_log.
...
def elbo_loss(recon, target, mu, log_var, beta=1.0):
    ...
    recon_loss = 1000.0 * nn.functional.mse_loss(
        recon, target, reduction="mean"
    ) + 0.5 * (1 - ssim(recon, target))  # Calculate Reconstruction Loss
...
def kl_annealing(epoch, warmup_epochs=50, max_beta=0.1):
    return min(
        max_beta, (epoch / warmup_epochs) * max_beta
    )  # Calculates the rate of annealing
