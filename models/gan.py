...
class UpsampleBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, scale_factor: int = 2):
        ...
        self.upsample = nn.Sequential(
            ...
            # Standard convolution allows all channels to interact (Un-nerfed for ADA 6000)
...
class Generator(nn.Module):
    def __init__(self, latent_dim: int = 256, base_channels: int = 128, num_classes: int = 5, embed_size: int = 50):
        super().__init__()
        self.number_of_residual_blocks = 12
        self.number_of_upsample_blocks = 6
        self.base_channels = base_channels

        # cGAN: Embedding layer for the 5 clusters
        ...
        self.upsample = nn.Sequential(
            *[UpsampleBlock(self.base_channels, 2) for _ in range(self.number_of_upsample_blocks)] 
        )
...
class Critic(nn.Module):
    def __init__(self, base_channels: int = 128, num_classes: int = 5, image_size: int = 256):
        super().__init__()
